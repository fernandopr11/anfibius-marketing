...
export default function DataTable<T extends BaseEntity>({
  ...
  const hasActions =
    !readOnly && (onView || onManageFiles || onEdit || onDelete);
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
        {currentData.length === 0 ? (
          ...
          currentData.map((item) => (
            <div
              key={item.id}
              className="bg-white rounded-lg border border-gray-200 shadow-sm hover:shadow-md transition-shadow flex flex-col h-full"
            >
